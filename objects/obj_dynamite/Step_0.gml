...
if (place_meeting(x + hsp, y, obj_solid) || place_meeting(x + hsp, y + vsp, obj_destructibles) || (obj_explosion != obj_canonexplosion && place_meeting(x, y, obj_baddie)))
	image_xscale *= -1;
if (place_meeting(x + hsp, y + vsp, obj_destructibles) || place_meeting(x + hsp, y + vsp, obj_ratblock) || place_meeting(x + hsp, y + vsp, obj_ratblock1x1) || place_meeting(x + hsp, y + vsp, obj_metalblock) || place_meeting(x + hsp, y + vsp, obj_iceblock_breakable) || place_meeting(x + hsp, y + vsp, obj_tutorialtarget) || place_meeting(x + hsp, y + vsp, obj_tntblock) || (obj_explosion != obj_canonexplosion && place_meeting(x, y, obj_baddie)))
{
	instance_destroy();
	instance_create(x, y, obj_explosion);
}
