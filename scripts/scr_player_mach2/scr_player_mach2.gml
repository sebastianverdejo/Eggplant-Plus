function scr_player_mach2()
{
	...
	if character == "V"
	{
		if (state != states.machslide && state != states.UNKNOWN_1 && scr_solid(x + xscale, y) && (!scr_slope()) && grounded)
		{
			if skateboarding
				xscale *= -1;
			else
	...
	if input_buffer_slap > 0 && !key_up && !skateboarding && shotgunAnim == 0 && !global.pistol && character != "V"
	{
		input_buffer_slap = 0;
	...
	else if (input_buffer_slap > 0 && key_up && shotgunAnim == 0 && !skateboarding && (!global.pistol || !ispeppino) && character != "V")
	{
		input_buffer_slap = 0;
		state = states.punch;
		image_index = 0;
