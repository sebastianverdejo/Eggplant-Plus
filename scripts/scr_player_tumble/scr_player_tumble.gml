function scr_player_tumble()
{
	...
	var sndslide = fmod_event_create_instance("event:/sfx/vigilante/slide");
	fmod_event_instance_set_3d_attributes(sndslide, x, y);
	hsp = (xscale * movespeed) + (railmovespeed * raildir);
	move = key_right + key_left;
	mask_index = spr_crouchmask;
	if sprite_index == spr_playerV_divekickstart && movespeed < 15
	{
		fmod_event_instance_play(sndslide);
		flash = true;
		movespeed = 15;
		image_index = 0;
		image_speed = 0.15
	...
	if crouchslipbuffer > 0
		crouchslipbuffer--;
	if (!key_down && key_attack && grounded && state != states.bump && (sprite_index != spr_tumble && sprite_index != spr_tumbleend && sprite_index != spr_tumblestart) && !scr_solid(x, y - 16) && !scr_solid(x, y - 32) && sprite_index != spr_breakdance)
	{
		if crouchslipbuffer == 0
		{
	...
	if (!key_down && !key_attack && grounded && vsp > 0 && state != states.bump && (sprite_index != spr_tumble && sprite_index != spr_tumbleend && sprite_index != spr_tumblestart) && !scr_solid(x, y - 16) && !scr_solid(x, y - 32) && sprite_index != spr_breakdance)
	{
		if crouchslipbuffer == 0
		{
			if movespeed > 6
