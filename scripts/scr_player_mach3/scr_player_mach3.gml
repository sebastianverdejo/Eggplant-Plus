function scr_player_mach3() //gml_Script_scr_player_mach3
{
    ...
    switch character
    {
                ...
                if (!ispeppino)
                {
                    repeat (4)
                    {
                        with (instance_create((x + irandom_range(-40, 40)), (y + irandom_range(-40, 40)), obj_explosioneffect))
                        {
                            ...
                            image_speed = 0.34999999999999998
                        }
                    }
                }
            }
        ...
        case "V":
            ...
            if (scr_solid((x + sign(hsp)), y) && (!(place_meeting((x + sign(hsp)), y, obj_mach3solid))) && (!scr_slope()) && (scr_solid_slope((x + sign(hsp)), y) || place_meeting((x + sign(hsp)), y, obj_solid)) && (!(place_meeting((x + sign(hsp)), y, obj_destructibles))) && (!(place_meeting((x + sign(hsp)), y, obj_climbablewall))) && grounded)
            {
                var _bump = 1
                if _bump
                {
    ...
    if ((sprite_index == mach3_spr) || (sprite_index == spr_fightball))
        ...
        image_speed = 0.40000000000000002
    if (scr_check_superjump() && character != "V" && (fightball == 0) && (state == states.mach3) && grounded && (vsp > 0) && (sprite_index != spr_dashpadmach) && (!(place_meeting(x, y, obj_dashpad))))
    {
